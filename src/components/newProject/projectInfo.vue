<template>
    <div class="new-project-info-container">
        ...
        <div>
            ...
            <div class="save-btn" @click="(nowProjectId)?doEdit():doSave()">儲存</div>
        </div>
    </div>
</template>
...
<script>
    ...
    import {saveNewProject, checkProName, addProjectMember, getProjectInfo, updateProjectInfo} from "../../api";
        ...
        methods: {
            ...
            * */
            async getProjectInformation() {
                ...
                let result = await getProjectInfo(paramObj);
                console.log('取得單一專案資訊=>', result)
                if (result.result) {
                    let data = result.data
                    this.projectName = data.project_name
                    this.projectDescription = data.description
                    this.tagList = (data.tags.length > 0) ? JSON.parse(JSON.stringify(data.tags)) : ''
                }
            ...
            async doEdit() {
                ...
                if (checkInput) {
                    ...
                    let result = await updateProjectInfo(paramObj)
                    console.log('編輯專案=>', result);
                }
            ...
            checkInput() {
                ...
                if (this.userList.length === 0 || this.userList.members_info_list.length === 0) {
                    errMsg += '參與人員'
                    sts = false
                }
