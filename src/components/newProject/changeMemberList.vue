...
<script>
    ...
    import {getProjectMemberList} from '@/js/globalFunction'
    ...
    export default {
        ...
        mounted() {
            ...
            if (this.nowProjectId) {
                this.getNowProjectUser(this.nowProjectId)
            }
        ...
        methods: {
            ...
            * */
            async getProjectUserBySelect(id) {

                if (id) {
                    let project_id = this.selectedProject
                    let result = await getProjectMemberList(project_id, false)
                    console.log(result)
            ...
            async getNowProjectUser() {
                let project_id = this.nowProjectId
                let result = await getProjectMemberList(project_id, false)
                console.log(result)
                this.joinedMemberList = this.doformat(result.user_list)
            },

            doformat(data) {
                let newarr = data.map((item) => {
                    let result = Object.assign({}, this.allMemberList.find((member) => member.user_id == item.user_id))
                    if (item.permission.length > 0) {
                        item.permission.forEach((permission) => {
                            result.label = (permission == 'labeler') ? true: false
                            result.confirm = (permission == 'confirmer') ? true : false
                        })
                    }
                    return result
                })
            ...
            * */
            ...
            moveSelectedtoLeft() {
                ...
                this.joinedMemberSelection.forEach(item => {
                    let index = this.joinedMemberList.findIndex(i => i.id === item.id)
